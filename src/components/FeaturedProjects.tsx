...
const FeaturedProjects = () => {
  const featuredProjects = [{
    ...
    description: "Time-series feature engineering and ML classification to categorize household energy usage into low/medium/high consumption patterns.",
  ...
  return <section className="py-24 bg-background" id="projects">
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
          ...
          delay: index * 0.1
              ...
              <div className="h-full glass-card rounded-2xl overflow-hidden hover-lift shadow-pastel gradient-border">
                ...
                {project.imageUrl && <div className="px-6 pt-6">
                    <div className="relative overflow-hidden rounded-xl border border-border/60 bg-muted/30">
                      <img src={project.imageUrl} alt={`${project.title} time-series chart placeholder`} loading="lazy" className="h-32 w-full object-cover opacity-80 transition-opacity duration-300 group-hover:opacity-100" />
                ...
                <div className="p-6">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      {project.githubUrl ? <a href={project.githubUrl} target="_blank" rel="noreferrer" className="p-2 rounded-full hover:bg-pastel-lavender/20 transition-colors" aria-label={`Open ${project.title} on GitHub`}>
                          <Github className="w-4 h-4 text-muted-foreground" />
                        </a> : <button className="p-2 rounded-full hover:bg-pastel-lavender/20 transition-colors">
                          <Github className="w-4 h-4 text-muted-foreground" />
                        </button>}
                  ...
                  <div className="flex flex-wrap gap-2">
                    {project.technologies.map((tech, techIndex) => <span key={techIndex} className="px-2 py-1 text-xs rounded-lg bg-muted text-muted-foreground">
                        {tech}
                      </span>)}
                  </div>
                </div>
        ...
        once: true
          ...
          <Link to="/projects">
            <Button variant="outline" size="lg" className="rounded-full px-8 group hover:bg-pastel-lavender/20">
